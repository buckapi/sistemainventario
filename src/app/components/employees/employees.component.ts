...
export class EmployeesComponent {
  pb = new PocketBase('https://db.buckapi.lat:8095');
 ...
 /*  deleteEmployee(employeeId: string) {
    ...
    deleteEmployee(employeeId: string) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            await this.pocketbase.collection('employees').delete(employeeId);
            console.log('Empleado eliminado exitosamente');
            this.realtimeEmployees.employees$; // Actualiza la lista de empleados
            this.cancelEdit();
            Swal.fire(
              '¡Eliminado!',
              'El empleado ha sido eliminado.',
